        ...
        try {
            ...
            homescreenManager.managedObject.mediaObject.addEventListener(
                ...
                function () {
                    ...
                    item.setAttribute(
                        "onclick",
                        `send${type}Path("${esSource}")`
                    );

                    let progressBar = document.getElementById(
                        `${type}-progress-bar-${id}`
                    );
                    if (progressBar) {
...
function sendvideoPath(itemSource) {
    ...
    ipcRenderer.send("save-video-link", itemSource);
    console.log(
        `I successfully sent the video link. the link I send was ${itemSource}`
    );
...
function sendaudioPath(itemSource) {
    console.log(`The link before sending was ${itemSource} `);
    ipcRenderer.send("save-audio-link", itemSource);
    console.log(
        `I successfully sent the audio link. the link I send was ${itemSource}`
    );
